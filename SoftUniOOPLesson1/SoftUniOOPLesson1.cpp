...
void rust::copyM(char(&source)[10][10], char(&dest)[10][10]) {
...
void rust::applyRust(char(&matrix)[10][10], char(&finalMatrix)[10][10]) {
	for (int row = 0; row < 10; row++) {
		for (int col = 0; col < 10; col++)
		{
			if (matrix[row][col] == '!') {
...
class matchingLocations {
	public:
		void matchingLocationsFinal();
};
void matchingLocations::matchingLocationsFinal() {
	vector<string>names;
...
int main()
{
	...
	location.matchingLocationsFinal();
